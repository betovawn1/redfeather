...
gulp.task( 'hbs', function( done ) {
	...
	return gulp
		...
		.pipe( data( function( file ) {
			return require(
				// file.path.replace( /(temp\/)(\w|-)+(\/)((\w|-)+)(\.hbs)/g, 'source/data/'+ orderJsonToRead +'/$4.json' ) // Old
				file.path.replace('temp','source').replace('en-US','data\\vtex').replace('pt-BR','data\\vtex').replace('.hbs','.json') // Novo WDT para Windows
			);
		} ) )
		.pipe( hbStream )
		.pipe( gulp.dest( 'temp' ) )
